...
class AIService:
    ...
    def get_ai_response(self, user_message):
        ...
        for i in range(len(self.claude_api_keys)):
            ...
            if key:
                try:
                    response = self._call_claude(key, user_message)
        ...
        for i in range(len(self.gemini_api_keys)):
            ...
            if key:
                try:
                    response = self._call_gemini(key, user_message)
        ...
        for i in range(len(self.groq_api_keys)):
            ...
            if key:
                try:
                    response = self._call_groq(key, user_message)
        ...
        for i in range(len(self.deepseek_api_keys)):
            ...
            if key:
                try:
                    response = self._call_deepseek(key, user_message)
        ...
        for i in range(len(self.openai_api_keys)):
            ...
            if key:
                try:
                    response = self._call_openai(key, user_message)
        ...
        demo_responses = [
            f"Интересный вопрос: '{user_message}'. Это демо-режим !",
            f"Я получил ваше сообщение: '{user_message}'. Это демо-версия !",
            f"Демо-ответ на: '{user_message}'. Сервис в данное время не работает !"
        ]
        
        import random
        return random.choice(demo_responses)
    
    def _call_claude(self, api_key, user_message):
        import anthropic
    ...
    def _call_gemini(self, api_key, user_message):
    ...
    def _call_groq(self, api_key, user_message):
    ...
    def _call_deepseek(self, api_key, user_message):
    ...
    def _call_openai(self, api_key, user_message):
        from openai import OpenAI
